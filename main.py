...
if __name__ == '__main__':
    ...
    parser.add_argument('-O', action='store_false', help="equals --fp16 --cuda_ray")
    ...
    parser.add_argument('--test_interval', type=int, default=50, help="test on the test set every interval epochs")
    parser.add_argument('--workspace', type=str, default='out/test')
    ...
    parser.add_argument('--freeze_guidance', action='store_true', help="freeze guidance embadding during dmtet tuning")
    ...
    parser.add_argument('--head_recon', type=float, default=0, help="head patch reconstrction")
    parser.add_argument('--hand_recon', type=float, default=0, help="hand patch reconstrction")
    ...
    parser.add_argument('--uvd_proj', action='store_true', help="use uvd in nerf")
    parser.add_argument('--pose_path', default=None, help="dmtet driven target pose data")
    parser.add_argument('--gt_smplx', default=None, help="dmtet mesh gt pose data")
    parser.add_argument('--handy_path', default=None, help="handy model path")
    parser.add_argument('--smplx_path', default=None, help="smplx model path")
    parser.add_argument('--gender', default=None, help="smplx model gender")
    parser.add_argument('--flat_hand_mean', action='store_true', help="smplx model flat hand mean")
    parser.add_argument('--mano_path', default=None, help="mano model path")
    parser.add_argument('--eval_metrics', action='store_true', help="whether eval 24 views' metrics")
    parser.add_argument('--eval_supl', action='store_true', help="whether generate 100 views' video")
    parser.add_argument('--hand_dst_shape', default=None, help="shape parameters of the hand to be driven, use zero shape if None")
    parser.add_argument('--real_shape_scale', default=None, help="the ratio of real hand mesh to template mano mesh")
    parser.add_argument('--various_pose', action='store_true', help="different poses for different views")
    parser.add_argument('--tpose', action='store_true', help="tpose as base pose")
    parser.add_argument('--hand_tpose', action='store_true', help="tpose as base pose to drive hand")
    parser.add_argument('--mesh_rotate', action='store_true', help="camera fixed and mesh rotate")
    parser.add_argument('--expression', action='store_true', help="expression deformation")
    parser.add_argument('--tets', default=None, help="dmtet tets grid")
    parser.add_argument('--denormalize', action='store_true', help="DON'T set weights of different views in zero123 to be normalized")
    parser.add_argument('--oneviewloss', action='store_true', help="if True, only calculate loss with one ref view, only works when set denormalize")
    parser.add_argument('--real_hand_pose', action='store_true', help="if True, use alignment matrix of real_hand data")
    ...
    parser.add_argument('--known_view_scale', type=float, default=2.0, help="multiply --h/w by this for known view rendering")
    ...
    parser.add_argument('--theta_range', type=float, nargs='*', default=[0, 180], help="training camera range along the polar angles (i.e. up and down). See advanced.md for details.")
    ...
    parser.add_argument('--view_angle', type=int, default=0, help="view angle for animation")


    ### regularizations
    parser.add_argument('--lambda_entropy', type=float, default=1, help="loss scale for alpha entropy")
    parser.add_argument('--lambda_opacity', type=float, default=0, help="loss scale for alpha value")
    parser.add_argument('--lambda_orient', type=float, default=1, help="loss scale for orientation")
    ...
    parser.add_argument('--lambda_rgb', type=float, default=100, help="loss scale for RGB")
    ...
    parser.add_argument('--lambda_3d_normal_smooth', type=float, default=10, help="loss scale for 3D normal image smoothness")
    parser.add_argument('--lambda_normal_sds', type=float, default=0, help="loss scale for color smoothness")
    parser.add_argument('--lap_vn', action='store_true', help="laplacian loss for normal vectors")
    parser.add_argument('--lambda_sds', type=float, default=1, help="loss scale for diffusion model loss")
    ...
    # if eval_metrics is True, forcibly set number of render imgs to 24
    if opt.eval_metrics:
        opt.dataset_size_test = 24
        opt.test = True
    
    if opt.eval_supl:
        opt.dataset_size_test = 100
        opt.test = True

    assert opt.handy_path is None or opt.mano_path is None, "Specify at most one model to be driving template"
    ...
    if opt.dmtet:
        ...
        ### set opt.progressive_view as config does ###
    ...
    if opt.six_views:
        ...
        if opt.backbone == 'vanilla':
            ...
            # scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.1 ** min(iter / opt.iters, 1))
            scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.8 ** (iter // opt.lr_iters))
            print('reduce lr by 0.8 for every{}'.format(opt.lr_iters))
