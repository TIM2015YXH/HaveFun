...
class Augmentation(object):
    ...
    def __call__(self, img):
        return self.augment(img).clip(0, 255)


def crop_roi(img, bbox, out_sz, padding=(0, 0, 0), bodder_mode=cv2.BORDER_CONSTANT):
    if not isinstance(out_sz, tuple):
    ...
    crop = cv2.warpAffine(img, mapping, out_sz,
                          borderMode=bodder_mode,
                          borderValue=padding)
    return crop
...
class ColorAug(object):
    def __init__(self):
        self.seq = iaa.Sequential([
            ...
            #               else_list=[iaa.Dropout2d(p=0.5, nb_keep_channels=0)]
            #               ),
            iaa.SomeOf((1, None), [
                iaa.Multiply((0.6, 1.2), per_channel=0.2),
                iaa.ChangeColorTemperature((1000, 40000)),
                iaa.Cutout(nb_iterations=(1, 3), size=(0.1,0.15),fill_mode="constant", cval=(0, 255), fill_per_channel=0.5),
                iaa.Sharpen((0.0, 1.0)),
                iaa.KMeansColorQuantization(n_colors=(16, 256))]),
            # sharpen the image
            # iaa.Affine(scale=(0.6, 1.2))
            # iaa.Affine(scale=(0.8, 1.2),
            #    translate_percent={"x": (-0.1, 0.1), "y": (-0.1, 0.1)},
            #    shear=(-0.1, 0.1)),  # resize (affects segmaps)
            # iaa.ElasticTransformation(alpha=50, sigma=5)  # apply water effect (affects segmaps)
        ], random_order=True)

    def __call__(self, image):
        if random.randint(2) and False:
            return image
