...
class Renderer(nn.Module):
    def __init__(self, image_size=256, anti_aliasing=True, background_color=[0,0,0],
                 ...
                 light_direction=[0,1,0], device='cpu'):
        ...
        if self.camera_mode == 'projection':
            ...
            if isinstance(self.K, numpy.ndarray):
                self.K = torch.from_numpy(self.K).float().to(device)
            if isinstance(self.R, numpy.ndarray):
                self.R = torch.from_numpy(self.R).float().to(device)
            if isinstance(self.t, numpy.ndarray):
                self.t = torch.from_numpy(self.t).float().to(device)
            self.dist_coeffs = dist_coeffs
            if dist_coeffs is None:
                self.dist_coeffs = torch.FloatTensor([[0., 0., 0., 0., 0.]]).to(device)
