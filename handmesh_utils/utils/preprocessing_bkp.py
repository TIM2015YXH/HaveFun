...
def get_aug_config(exclude_flip):
    ...
    return scale, rot, do_flip


def augmentation(img, bbox, data_split, exclude_flip=False, input_img_shape=(256, 256), mask=None):
    if data_split == 'train':
        scale, rot, do_flip = get_aug_config(exclude_flip)
    else:
        scale, rot, do_flip = 1.0, 0.0, False

    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, input_img_shape)
    if mask is not None:
        mask_patch = cv2.warpAffine(mask, trans, (int(input_img_shape[1]), int(input_img_shape[0])), flags=cv2.INTER_LINEAR)
    else:
        mask_patch = None
    # img = np.clip(img * color_scale[None, None, :], 0, 255)
    return img, trans, inv_trans, rot, do_flip, input_img_shape[0]/(bbox[2]*scale), mask_patch/255
