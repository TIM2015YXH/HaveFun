...
class Handy(nn.Module):
    def __init__(self,
                 model_path,
                 device=None):
        ''' Handy model constructor'''
        super().__init__()
        dtype = torch.float32
        self.dtype = dtype
        self.device = device
        # create the Handy model
        if osp.isdir(model_path):
            model_fn = 'HANDY_RIGHT.{ext}'.format(ext='pkl')
            handy_path = osp.join(model_path, model_fn)
        else:
            handy_path = model_path

        assert osp.exists(handy_path), 'Path {} does not exist!'.format(
            handy_path)

        with open(handy_path, 'rb') as handy_file:
            data_struct = Struct(**pickle.load(handy_file,
                                               encoding='latin1'))
        data_struct.shapedirs = data_struct.shapedirs[...,:10]
        ...
        # Pose blend shape basis: 7231 x 3 x 135, reshaped to 7231*3 x 135
    ...
    def verts_transformations(self,
        ...
        L, vertices, delta_v = lbs(betas, poses, self.v_template,
    ...
    def verts_transformations_avatar(self,
                              ...
                              concat_joints=False):
        ''' Forward pass for SMPL model but also return transformation of each vertex
            this version does not use the beta of SMPL, but directly shaped verticies
        Parameters
        ----------
        betas: torch.tensor, optional, shape Bx10
                If given, ignore the member variable `betas` and use it
        ...
        if isinstance(transl, np.ndarray):
            transl = to_tensor(transl, self.dtype, self.device)
        L, vertices = my_lbs(v_shaped, poses, self.v_template,
                          self.shapedirs, self.posedirs,
                          self.J_regressor, self.parents,
                          self.lbs_weights, 
                          return_T=True, concat_joints=concat_joints)
    ...
    def forward_v_shaped(self,
                        ...
                        return_joints=False,
                        return_tensor=True):
        ''' Forward pass for SMPL model, but take shaped vertices as input
        '''
        bn = poses.shape[0]
        assert bn == 1
        if isinstance(poses, np.ndarray):
...
def handy_lbs(betas, pose, v_template, shapedirs, posedirs, joints, parents, lbs_weights, dtype=torch.float32, pose2rot=True):
    ''' Performs Linear Blend Skinning with the given shape and pose parameters
...
if __name__ == '__main__':
    ...
    betas[0, 0] = -2.0
    verts = smpl(pose, betas)
    save_mesh(verts, smpl.faces,'test_-2.ply')