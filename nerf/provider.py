...
from .utils import get_rays, safe_normalize, clip_bbox
import pickle
...
def rand_poses(size, device, opt, radius_range=[1, 1.5], theta_range=[0, 120], phi_range=[0, 360], return_dirs=False, angle_overhead=30, angle_front=60, uniform_sphere_rate=0.5):
    ...
    if random.random() < uniform_sphere_rate:
        ...
        # phi_choice = torch.tensor([np.pi / 2 * i for i in range(4)], device=device)
        # phis = phi_choice[torch.randint(0, len(phi_choice), (size,))]
        phis[phis < 0] += 2 * np.pi
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', H=256, W=256, size=100):
        ...
        self.pose_path = self.opt.pose_path
    ...
    def get_default_view_data(self):
        ...
        if self.opt.head_recon>0:
            H = W = 1024 if self.opt.dmtet else 512
            cx = H / 2
            cy = W / 2
            focal = H / (2 * np.tan(np.deg2rad(fov) / 2))
            intrinsics = np.array([focal, focal, cx, cy])
            ...
            rays_o_heads = []
            rays_d_heads = []
            for i, r in enumerate(kpt_paths):
                f = open(kpt_paths[i])
                data_kpt = json.load(f)
                bbox_head_size = H//4
                face_kpt_mean = np.array(data_kpt['face_kpt_mean'])*H
                face_bbox = face_kpt_mean.astype(np.int32) - bbox_head_size//2

                face_bbox = clip_bbox(face_bbox, H-bbox_head_size)
            
                rays_head = get_rays(poses[i:i+1,...], intrinsics, H, W, -1, head_ray=True, bbox=face_bbox)
                rays_o_heads.append(rays_head['rays_o'])
                rays_d_heads.append(rays_head['rays_d'])
            rays_o_heads = torch.stack(rays_o_heads, dim=0).squeeze(1)
            rays_d_heads = torch.stack(rays_d_heads, dim=0).squeeze(1)
            # data.update({
            #     'rays_o_head': rays_head['rays_o'],
            #     'rays_d_head': rays_head['rays_d'],
            # })
            data.update({
                'rays_o_head': rays_o_heads,
                'rays_d_head': rays_d_heads,
            })
        
        if self.opt.hand_recon>0:
            H = W = 1024 if self.opt.dmtet else 1024
            cx = H / 2
            cy = W / 2
            focal = H / (2 * np.tan(np.deg2rad(fov) / 2))
            intrinsics = np.array([focal, focal, cx, cy])
            ...
            for i, r in enumerate(kpt_paths):
                ...
                data_kpt = json.load(f)
                bbox_hand_size = H//8
                hand_left_kpt_mean = np.array(data_kpt['hand_left_kpt_mean'])*H
                hand_left_bbox = hand_left_kpt_mean.astype(np.int32) - bbox_hand_size//2
                hand_left_bbox = clip_bbox(hand_left_bbox, H-bbox_hand_size)
            
                rays_hand = get_rays(poses[i:i+1,...], intrinsics, H, W, -1, hand_ray=True, bbox=hand_left_bbox)
                rays_o_hand_left.append(rays_hand['rays_o'])
                rays_d_hand_left.append(rays_hand['rays_d'])

                hand_right_kpt_mean = np.array(data_kpt['hand_right_kpt_mean'])*H
                hand_right_bbox = hand_right_kpt_mean.astype(np.int32) - bbox_hand_size//2
                hand_right_bbox = clip_bbox(hand_right_bbox, H-bbox_hand_size)
            
                rays_hand = get_rays(poses[i:i+1,...], intrinsics, H, W, -1, hand_ray=True, bbox=hand_right_bbox)
                rays_o_hand_right.append(rays_hand['rays_o'])
                rays_d_hand_right.append(rays_hand['rays_d'])
            
            rays_o_hand_left = torch.stack(rays_o_hand_left, dim=0).squeeze(1)
            rays_d_hand_left = torch.stack(rays_d_hand_left, dim=0).squeeze(1)
            rays_o_hand_right = torch.stack(rays_o_hand_right, dim=0).squeeze(1)
            rays_d_hand_right = torch.stack(rays_d_hand_right, dim=0).squeeze(1)
            # data.update({
            #     'rays_o_head': rays_head['rays_o'],
            #     'rays_d_head': rays_head['rays_d'],
            # })
            data.update({
                'rays_o_hand_left': rays_o_hand_left,
                'rays_d_hand_left': rays_d_hand_left,
                'rays_o_hand_right': rays_o_hand_right,
                'rays_d_hand_right': rays_d_hand_right,
            })
    ...
    def collate(self, index):
        ...
        if self.training:
            ...
            thetas = torch.cat((angle_45, angle_90, angle_135)).reshape(-1, 1).to(self.device)

            ph = [x for x in range(-90, 270, 45)]
            phis = torch.FloatTensor(ph).repeat(3).reshape(-1, 1).to(self.device)
            ...
            radius = torch.FloatTensor([self.opt.default_radius]).to(self.device)
            poses, dirs = circle_poses(self.device, radius=radius, theta=thetas[index[0]], phi=phis[index[0]], return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front)
            # fixed focal
            fov = self.opt.default_fovy

        elif self.opt.eval_supl:
            angle_90 = torch.full((100,), 90.0)
            # thetas = torch.cat((, angle_90, )).reshape(-1, 1).to(self.device)
            thetas = angle_90.reshape(-1, 1).to(self.device)
            # thetas = torch.cat((angle_135, angle_90, angle_45)).reshape(-1, 1).to(self.device)

            ph = [float(x)/10.0 for x in range(0, 3600, 36)]
            # ph = [x for x in range(0, 360, 45)]
            phis = torch.FloatTensor(ph).reshape(-1, 1).to(self.device)
        ...
        elif self.opt.pose_path is not None:
            thetas = torch.FloatTensor([90]).to(self.device)
            if self.opt.view_angle==360:
                phis = torch.FloatTensor([(index[0] / self.size) * 360]).to(self.device)
            else:
                phis = torch.FloatTensor([self.opt.view_angle]).to(self.device)
            # 
            ...
            if self.pose_path is None:
                ...
                thetas = torch.FloatTensor([((index[0] + 1) / self.size) * 360]).to(self.device)
            phis = torch.FloatTensor([(index[0] / self.size) * 360]).to(self.device)
            # if self.opt.smplx_path:
            #     thetas = torch.FloatTensor([90]).to(self.device)
            #     phis = torch.FloatTensor([90]).to(self.device)
            radius = torch.FloatTensor([self.opt.default_radius]).to(self.device)
            poses, dirs = circle_poses(self.device, radius=radius, theta=thetas, phi=phis, return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front)
        ...
        if self.opt.head_recon>0:
            rays_head = get_rays(poses, intrinsics, self.H, self.W, -1, head_ray=True)
            data.update({
                'rays_o_head': rays_head['rays_o'],
                'rays_d_head': rays_head['rays_d'],
            })
        ...
        #     hand_pose = pose_data[index[0]].reshape(1, 48)
        #     __handpose = {'hand_pose': hand_pose}
        #     data.update(__handpose)

        dst_pose = None
        if self.pose_path is not None:
            if '.npy' in self.pose_path:
                pose_data = np.load(self.pose_path)
                dst_pose = pose_data[index[0]]
                __handpose = {'dst_pose': dst_pose}
                data.update(__handpose)
            elif '.pkl' in self.pose_path:
                pose_data = pickle.load(open(self.pose_path, 'rb'))
                dst_pose = {'dst_pose': pose_data}
                data.update(dst_pose)
        return data
